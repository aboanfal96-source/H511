...
H('التكملة الفراكتالية — للهدف الثاني فقط');
{
  ...
  ok('ولا بعد إسقاط البنيوي دون الحدّ', /if\(!kept\.some\(t=>t\.kind==='structural'\)\)kept=\[\];/.test(src));
}
...
H('أحكام الدخول تقول ما قيس');
{
  ...
  if (now) {
    ok('عند بنية: يذكر نصفي الفترة ويقول لا أفضلية ثابتة', /2022–2024/.test(now) && /2024–2026/.test(now) && /لا أفضلية ثابتة/.test(now), now.slice(-160));
    ok('عند بنية: لم يعد يبدأ بـ«ادخل الآن»', !/ادخل الآن/.test(now));
    ok('لا سالب مزدوج في النصّ', !/خسر -/.test(now));
  }
}
