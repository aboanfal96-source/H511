...
(async () => {
  ...
  const orphan = tl.filter(s => (TL[s].targets || []).length && !(TL[s].targets || []).some(t => t.kind === 'structural')).length;
  const fracFirst = tl.filter(s => TL[s].targets && TL[s].targets[0] && TL[s].targets[0].kind === 'fractal').length;
  log(`أهداف بلا أي مستوى بنيوي: ${orphan} · هدف أول فراكتالي أقرب من البنيوي: ${fracFirst} · خطط مجدية ${tl.filter(s => TL[s].viable).length}`);
